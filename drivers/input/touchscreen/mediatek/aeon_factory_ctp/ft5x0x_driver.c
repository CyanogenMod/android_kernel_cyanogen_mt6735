...
static const struct i2c_device_id ft5x0x_tpd_id[] = {{"aeon_factory_ctp", 0}, {} };
static const struct of_device_id ft5x0x_dt_match[] = {
	{.compatible = "mediatek,factory_touch"},
	{},
};
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t)tpd_eint_interrupt_handler,
					IRQF_TRIGGER_PROBE, TPD_DEVICE, NULL);
		if (ret > 0)
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	unsigned int factory_pin_id;
	factory_pin_id = aeon_gpio_get("aeon_factory_id");
	printk("tpd_probe factory_pin_id=%d\n", factory_pin_id);
	...
	client->addr = 0x70 >>1; //lijin add 20151224
	i2c_client = client;
    ...
    if ((i2c_smbus_read_i2c_block_data(i2c_client, 0xA3, 1, &data)) < 0) {
		...
		TPD_DMESG("I2C transfer error,data=0x%x, line: %d\n", data, __LINE__);
    }
...
static int tpd_local_init(void)
{
	...
	TPD_DMESG("aeon_factory_ctp Focaltech FT5x0x I2C Touchscreen Driver...\n");
...
{
	TPD_DMESG("aeon_factpry_ctp MediaTek FT5x0x touch panel driver init\n");
	//tpd_get_dts_info();
#ifdef TPD_HAVE_BUTTON	
