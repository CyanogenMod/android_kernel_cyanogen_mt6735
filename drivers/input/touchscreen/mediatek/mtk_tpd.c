...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	if(strcmp(tpd_drv->tpd_device_name, "aeon_factory_ctp") == 0) {
#else
...
#ifdef AEON_FACTORY_MODULE
	if(strcmp(tpd_driver_list[0].tpd_device_name, "aeon_factory_ctp")) {
		//ASSERT(0);
	}
	for (i = 0; i < TP_DRV_MAX_COUNT; i++) {
#else
...
#ifdef AEON_FACTORY_MODULE
    if(strcmp(g_tpd_drv->tpd_device_name, "aeon_factory_ctp") == 0)
	{
		TPD_RES_X=480;
		TPD_RES_Y=800;
		tpd_mode_keypad_tolerance = TPD_RES_X*TPD_RES_X/1600;
