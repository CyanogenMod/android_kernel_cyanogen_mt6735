		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("4h5 daling i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("4h5 daling Read sensor id fail, id: 0x%x,0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		//i++;
		//retry = 2;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	// long long *feature_return_para=(unsigned long long *) feature_para;
