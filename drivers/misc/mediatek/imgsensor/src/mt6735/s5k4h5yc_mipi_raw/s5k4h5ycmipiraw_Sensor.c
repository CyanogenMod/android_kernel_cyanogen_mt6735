...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	return 0;//write_cmos_sensor(0x0104, 0x00); 
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			printk("===lanhai %s *sensor_id = %x===\n", __func__, *sensor_id);
			//*sensor_id = *sensor_id + 1;
            #if defined(S5K4h5yc_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_MIPI_read_otp_vendor_id() == 0x03) {
			  	//*sensor_id = 0x485B;
				*sensor_id = *sensor_id + 1;
				printk("===lanhai 11 %s *sensor_id = %x===\n", __func__, *sensor_id);
			  }
			  else
		...
		do {
			...
			printk("===lanhai %s sensor_id = %x===\n", __func__, sensor_id);
			  ...
			  if(sensor_id == 0x485B && S5K4h5yc_MIPI_read_otp_vendor_id() == 0x03) {
			  	//sensor_id = 0x485B;
				sensor_id = sensor_id + 1;
				printk("===lanhai 22 %s *sensor_id = %x===\n", __func__, sensor_id);
			  }
			  else
			    sensor_id = 0xFFFFFFFF;
		#else
