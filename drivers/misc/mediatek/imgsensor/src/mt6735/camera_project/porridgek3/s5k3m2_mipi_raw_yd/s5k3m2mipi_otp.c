...
#define S5K3M2_DEBUG
#ifdef S5K3M2_DEBUG
	#define S5K3M2_DB(fmt, arg...) pr_debug("[S5K3M2_OTP] ",  fmt, ##arg)
#else
...
kal_uint8 get_otp_AWB_flag(kal_uint8 groupIdx)
{
    ...
    printk("S5K3M2 zcw++: flag_AWB[group %d] := 0x%02x \n", groupIdx, flag_AWB );
    //return ((flag_AWB&0xC0)>>6);
    return flag_AWB;
}
...
kal_uint8 get_otp_LSC_flag(kal_uint8 groupIdx)
{
    ...
    printk("S5K3M2 zcw++: page[%d] flag_LSC := 0x%02x \n", groupIdx,flag_LSC );
...
kal_uint8 get_otp_AF_flag(kal_uint8 groupIdx)
{
    ...
    printk("S5K3M2 zcw++: Group[%d] flag_AF = 0x%02x \n", groupIdx,flag_AF );
	return flag_AF;
}
...
void get_otp_module_info(kal_uint8 groupIdx)
{
	...
	printk("S5K3M2 zcw++: Module ID = %d.\n",module_id);
	printk("S5K3M2 zcw++: Date Year = %d.\n",year);
    printk("S5K3M2 zcw++:        month = %d.\n",month);
    printk("S5K3M2 zcw++:        day= %d.\n",day);
    printk("S5K3M2 zcw++: Lens ID = %d.\n",lens_id);
    printk("S5K3M2 zcw++: VCM ID = %d.\n",vcm_id);
    printk("S5K3M2 zcw++: DriverIC ID = %d.\n",driverIC_id);
...
kal_bool otp_lsc_update(void)
{
    ...
    {
        //LSC correct
        printk("S5K3M2 zcw++: LSC Data Valid...Start Auto Correct\n");
...
kal_bool otp_wb_update(void)
{
    ...
    printk("S5K3M2 zcw++: [R_GAIN=%d],[G_GAIN=%d],[B_GAIN=%d] \n",R_GAIN, G_GAIN, B_GAIN);
...
int* get_otp_vcm_data(void)
{
	...
	for(i=0; i<2; i++)
	{
	    ...
	    {
	        printk("S5K3M2 zcw++ AF group [%d] is valid!\n",i);
	        break;
	    }
	}
	if(2 <= i)
	{
	    printk("S5K3M2 zcw++ AF Data Empty or Invalid!\n");
	    stop_read_otp();
	    return NULL;
	}
	else
	{
	    ...
	    */
	    printk("S5K3M2 zcw++ AF vcm[0]Inf=%d vcm[1]Macro=%d\n", S5K3M2_vcm_data[0],S5K3M2_vcm_data[1]);
        stop_read_otp();
...
kal_bool otp_update(void)
{
	...
	if(i==2)
	{
		printk("S5K3M2 zcw++: No OTP Data or OTP data is invalid!!\n");
		return 0;
	}
    get_otp_module_info(i);
    if(MID != TRULY_ID)
	{
		printk("S5K3M2 zcw++: Not Truly Module!!!!\n");
		return 0;
	}
    get_otp_wb(i);
