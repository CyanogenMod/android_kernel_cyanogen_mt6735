...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x583e,0x06);//max gain 3x
	write_cmos_sensor(0x583f,0x02);//min gain 1x
	write_cmos_sensor(0x5840,0x00);//min Q
	write_cmos_sensor(0x5841,0x0d);//auto Q enable
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		if ((*otp_ptr).light_rg==0) {
			...
			rg = (*otp_ptr).rg_ratio * ((*otp_ptr).light_rg + 512) / 1024;//current_otp.light_rg
		}
		if ((*otp_ptr).light_bg==0) {
			// not light source information in OTP, light factor = 1
			bg = (*otp_ptr).bg_ratio;
		} else {
			bg = (*otp_ptr).bg_ratio * ((*otp_ptr).light_bg + 512) / 1024;//current_otp.light_bg
		}
		//calculate G gain
