...
#ifdef BUILD_LK
	printf("LK lcm_compare_id ili9881 tongxingda buffer[0] = 0x%x,buffer[1] = 0x%x,buffer[2] = 0x%x, lcd_id1=%d,lcd_id2=%d \n",buffer[0],buffer[1],buffer[2], lcd_id1, lcd_id2);
#else
	printk("Kernel lcm_compare_id ili9881 tongxingda buffer[0] = 0x%x,buffer[1] = 0x%x,buffer[2] = 0x%x, lcd_id1=%d,lcd_id2=%d \n",buffer[0],buffer[1],buffer[2], lcd_id1, lcd_id2);
#endif
	if((buffer[0]==0x00) && (lcd_id1 == 0) && (lcd_id2 == 1))
	{
		return 1;
	}
	else
...
LCM_DRIVER aeon_ili9881c_hd720_dsi_vdo_txd_8536_lcm_drv = 
{
    .name			= "aeon_ili9881c_hd720_dsi_vdo_txd_8536",
