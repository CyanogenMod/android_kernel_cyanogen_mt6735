...
{0x6F,1,{0x43}},                // reg vcl + pumping ratio VGH=4x VGL=-2.5x
...
static unsigned int lcm_compare_id(void)
{
	...
	read_reg_v2(0xDA, buffer, 3);
        ...
        lcd_id1 = aeon_gpio_get("aeon_lcd_id1");

        /*mt_set_gpio_mode(GPIO_LCD_ID_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_LCD_ID_PIN, GPIO_DIR_IN);
        mt_set_gpio_pull_enable(GPIO_LCD_ID_PIN, 1);
        mt_set_gpio_pull_select(GPIO_LCD_ID_PIN, GPIO_PULL_UP);
        lcd_id1 = mt_get_gpio_in(GPIO_LCD_ID_PIN);
...
#ifdef BUILD_LK
	printf("LK lcm_compare_id ili9881 lide buffer[0] = 0x%x,buffer[1] = 0x%x,buffer[2] = 0x%x,lcd_id1=%d \n",buffer[0],buffer[1],buffer[2],lcd_id1);
#else
	printk("Kernel lcm_compare_id ili9881 lide buffer[0] = 0x%x,buffer[1] = 0x%x,buffer[2] = 0x%x,lcd_id1=%d\n",buffer[0],buffer[1],buffer[2],lcd_id1);
#endif
	if((buffer[0]==0x00) && (lcd_id1 == 1))
	{
		return 1;
	}
	else
...
LCM_DRIVER aeon_ili9881c_hd720_dsi_vdo_8630_lead_lcm_drv = 
{
    .name			= "aeon_ili9881c_hd720_dsi_vdo_8630_lead",
